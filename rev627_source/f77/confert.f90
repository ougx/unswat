...
!! apply manure
  ...
  IF (cfrt_kg(j) > 0.) THEN
    ...
    IF (cswat == 0 .OR. cswat == 1) THEN
      sol_no3(l,j) = sol_no3(l,j) + cfrt_kg(j) * (1. - fnh3n(it)) * fminn(it)
      sol_fon(l,j) = sol_fon(l,j) + cfrt_kg(j) * forgn(it)
      sol_nh3(l,j) = sol_nh3(l,j) + cfrt_kg(j) * fnh3n(it) * fminn(it)
      sol_solp(l,j) = sol_solp(l,j) + cfrt_kg(j) * fminp(it)
      sol_fop(l,j) = sol_fop(l,j) + cfrt_kg(j) * forgp(it)
    END IF
...
!X1 fertilizer attributed to fresh carbon & nitrogen pool
      ...
      sol_lsn(l,j) = sol_lsn(l,j) + x1 *forgn(it) -zz
      xz = x1 *orgc_f-xxx
