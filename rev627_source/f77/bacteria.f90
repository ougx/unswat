...
blps = 0.
bpq = bactpq(j)
bactpq(j) = bactpq(j) * EXP(-theta(wpq20,thbact,tmpav(j))) - bactminp
bactpq(j) = MAX(0., bactpq(j))
IF (bactpq(j) < bactminp) bactpq(j) = 0.
blpq = bactlpq(j)
bactlpq(j) = bactlpq(j) * EXP(-theta(wlpq20,thbact,tmpav(j))) - bactminlp
bactlpq(j) = MAX(0., bactlpq(j))
IF (bactlpq(j) < bactminlp) bactlpq(j) = 0.
bps = bactps(j)
bactps(j) = bactps(j) * EXP(-theta(wps20,thbact,tmpav(j))) - bactminp
bactps(j) = MAX(0., bactps(j))
IF (bactps(j) < bactminp) bactps(j) = 0.
blps = bactlps(j)
bactlps(j) = bactlps(j) * EXP(-theta(wlps20,thbact,tmpav(j))) - bactminlp
bactlps(j) = MAX(0., bactlps(j))
IF (bactlps(j) < bactminlp) bactlps(j) = 0.

!! compute bacteria in the runoff
bactrop = bactpq(j) * surfq(j) / (sol_bd(1,j) * sol_z(1,j) * bactkdq)
bactrop = MIN(bactrop, bactpq(j))
bactrop = MAX(bactrop, 0.)
bactpq(j) = bactpq(j) - bactrop

bactrolp = bactlpq(j) * surfq(j) / (sol_bd(1,j) * sol_z(1,j) * bactkdq)
...
bactlchp = bactpq(j) * sol_prk(1,j) / ((conv_wt(1,j) / 1000.) * bactmx)
bactlchp = MIN(bactlchp, bactpq(j))
bactlchp = MAX(bactlchp, 0.)
bactpq(j) = bactpq(j) - bactlchp

bactlchlp = bactlpq(j) * sol_prk(1,j) / ((conv_wt(1,j) / 1000.) * bactmx)
