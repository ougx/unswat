...
REAL :: sdrain_vfs1, drain_vfs2, area_vfs1, area_vfs2, vfs_depth1,  &
    ...
    remove2, orgn_remove, surqno3_remove, partp_remove, solp_remove, sedtrap, xrem
...
!! calculate sediment loading over buffer area in kg/m^2
  vfs_sed1 = (sedyld(j) / hru_ha(j) * 1000 * drain_vfs1) / (area_vfs1 * 10000)
  vfs_sed2 = (sedyld(j) / hru_ha(j) * 1000 * drain_vfs2) / (area_vfs2 * 10000)
...
!! Based on measured data from literature
  ...
  sed_remove = (sed_remove1 * drain_vfs1 + sed_remove2 * drain_vfs2)/hru_ha(j)
...
!! Based on measured data from literature
  ...
  orgn_remove = (remove1 * drain_vfs1 + remove2 * drain_vfs2)/hru_ha(j)
...
!! Based on measured data from literature
  ...
  surqno3_remove = (remove1 * drain_vfs1 + remove2 * drain_vfs2)/hru_ha(j)
...
!!Based on measured data from literature
  ...
  partp_remove = (remove1 * drain_vfs1 + remove2 * drain_vfs2)/hru_ha(j)
...
!!  DP% = - 6.14 + 1.13 Runoff%
  ...
  solp_remove = (remove1 * drain_vfs1 + remove2 * drain_vfs2)/hru_ha(j)
