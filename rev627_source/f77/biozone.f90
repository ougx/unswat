...
sol_ul(bz_lyr,j)=sol_por(bz_lyr,j)*bz_thk(j)-plqm(j) /(bio_bd(j)*10.)

IF(sol_ul(bz_lyr,j) <= sol_fc(bz_lyr,j)) THEN
...
rplqm = (rmort - rslg) + coeff_plq(j) * qin * spttssconcs(isp) / (1000. * bza)
rplqm = MAX(0.,rplqm)
...
xx = 10.* sol_st(bz_lyr,j) * bza / (qin + 10.* sol_st(bz_lyr,j) * bza)
...
svolp = (sol_z(nly,j) - bz_z(j)) * bza * 10. !m3,
 !m x adsorption amnt: linear isotherm, McCray 2005
...
solpsorb = 1.6 * 1.e-3 * solpsorb * svolp * (1-sol_por(bz_lyr,j)) !kgP sorption potential
 !ch ck if max. P sorption is reached
...
IF(curyr>nyskip) THEN
  ...
  WRITE(173,1000) ihru,iyr,iida,precipday,qout,sol_ul(bz_lyr,j),  &
      sol_st(bz_lyr,j),sol_fc(bz_lyr,j),n1,n2,n3,n4,n5,n6, n7,n8,p1,p2,p3,p4
END IF
...
wshd_seporgn = wshd_seporgn + xx * sptorgnconcs(isp) * rtof * hru_dafr(j)
wshd_sepfon = wshd_sepfon + xx * sptorgnconcs(isp)* (1.-rtof) * hru_dafr(j)
wshd_seporgp = wshd_seporgp + xx * sptorgps(isp) * rtof * hru_dafr(j)
wshd_sepfop = wshd_sepfop + xx * sptorgps(isp) * (1.-rtof) * hru_dafr(j)
