...
!! a ded for Total N (org N + no3 + no2 + nh4 outs) to output.rch gsm 10/17/2011
  pdvar(43) = rchmono(7,j)+ rchmono(16,j) + rchmono(35,j) +  &
      rchmono(33,j)                                                  !! Total N  &
!! a ded for Total P (org P + sol p outs)to output.rch gsm 10/17/2011
  pdvar(44) = rchmono(9,j) + rchmono(18,j)                        !! Total P  &
!! a ded NO3 Concentration to output.rch (for daily only) gsm 10/26/2011
  ...
  IF (ipdvar(1) > 0) THEN
    ...
    END IF
  ELSE  &
!  i crease to 44 in loops below from 42 gsm 10/17/2011
    IF (iscen == 1 .AND. isproj == 0) THEN
      WRITE (7,5000) j, subgis(j), mo_chk, rch_dakm(j),  &
          (pdvar(ii), ii = 1, 44), j
    ELSE IF (isproj == 1) THEN
