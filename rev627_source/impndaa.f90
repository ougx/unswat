...
!! calculate nutrient concentrations
    ...
    IF (pnd_vol(j) > 1.) THEN
      ...
      ano3_ppm = 1000. * (pnd_no3(j) + pnd_no3s(j) + pnd_no3g(j)) / pnd_vol(j)
      minp_ppm = 1000. * (pnd_solp(j)+pnd_psed(j)+pnd_solpg(j)) / pnd_vol(j)
      chla_ppm = 1000. * pnd_chla(j) / pnd_vol(j)
    END IF
    IF (wet_vol(j) > 1.) THEN
      orgn_ppw = 1000. * wet_orgn(j) / wet_vol(j)
      orgp_ppw = 1000. * wet_orgp(j) / wet_vol(j)
      ano3_ppw = 1000. * (wet_no3(j) + wet_no3s(j) + wet_no3g(j)) / wet_vol(j)
      solp_ppw = 1000. * (wet_solp(j)+wet_solpg(j)+wet_psed(j)) / wet_vol(j)
